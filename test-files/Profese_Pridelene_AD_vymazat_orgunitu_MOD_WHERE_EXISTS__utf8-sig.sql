...
/*    --------------------------- Pridelene AD profese, rn=1 nejstarsi zaznam s disable_auto_remove
    prof_tmp as (select pp.*
            ...
            , TEST_ATTR_A ta  -- KOMENTAR ta (pridano pro potreby ladeni kodu) 
            , row_number() over ( partition by pp.id_profession, pp.id_person 
                                  order by pp.disable_auto_remove desc, pp.ins_ts asc, pp.orgunitid) as rna    -- KOMENTAR rna (pridano pro potreby ladeni kodu)
            , row_number() over ( partition by pp.id_profession, pp.id_person 
                                  order by pp.disable_auto_remove desc, pp.ins_ts asc, pp.orgunitid) as rnb    -- KOMENTAR rnb (pridano pro potreby ladeni kodu)
            , TEST_ATTR_B tb  -- KOMENTAR tb (pridano pro potreby ladeni kodu)
